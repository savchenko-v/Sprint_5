...
class TestLogin:
    ...
    def test_login_main_page(self, driver):
        ...
        email_input.send_keys(LoginData.AUTH_EMAIL)

        password_input = driver.find_element(*Locators.PASSWORD_INPUT_FIELD)
        password_input.send_keys(LoginData.AUTH_PASSWORD)

        submit_button = driver.find_element(*Locators.LOGIN_SUBMIT_BUTTON)
        submit_button.click()

        WebDriverWait(driver, TimeData.WAIT_TIME).until(EC.visibility_of_element_located(Locators.ORDER_BUTTON))
        ...
        assert driver.current_url == UrlData.BURGERS_MAIN_URL and order_button.is_displayed()
    ...
    def test_login_register_page(self, driver):
        driver.get(UrlData.REGISTER_URL)
    ...
    def test_login_forgot_password_page(self, driver):
        driver.get(UrlData.FORGOT_PASSWORD_PAGE_URL)
